...
\newcommand{\ftfs}{5}       % frequência de amostragem f_s [Hz] (parâmetro fixo do estudo)
...
\input{topicos/intro}
\input{topicos/params}    % parâmetros fixos — vêm antes da teoria
\input{topicos/teoric}    % abertura da fundamentação teórica
\input{topicos/ideal}     % método ideal: geral -> parâmetros fixos -> reconstrução
\input{topicos/flattop}   % método topo-plano
